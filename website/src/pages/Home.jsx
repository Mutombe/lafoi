    ...
    <section ref={sectionRef} className="relative bg-lafoi-dark overflow-hidden">
      ...
      <div className="relative grid lg:grid-cols-2 min-h-[88vh] lg:min-h-[92vh]">
        ...
        >
          ...
          <div className="absolute top-6 right-6 lg:top-8 lg:right-8 inline-flex items-center gap-2 px-3 py-1.5 rounded-sm bg-white/12 backdrop-blur-md border border-white/20">
      ...
      <div className="relative max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10">
        ...
        <div className="mt-12 lg:mt-16 space-y-12 lg:space-y-16">
          {stretchProducts.map((p) => (
            <FinishRow key={p.slug} p={p} blurb={finishMeta[p.finish] || p.shortDesc} />
          ))}
        </div>
...
/* One ceiling-type row: a horizontally-scrollable strip of that finish's photos,
   with prev/next buttons at the bottom for mouse users (mobile uses touch). */
function FinishRow({ p, blurb }) {
  ...
  return (
    <div>
      ...
      <div className="flex items-end justify-between gap-4 mb-4">
        <div className="min-w-0">
          <div className="flex items-baseline gap-3">
            <h3 className="font-display font-light text-lafoi-dark text-2xl sm:text-3xl leading-none">{p.finish}</h3>
            <span className="font-sora text-[10px] tracking-[0.26em] uppercase text-lafoi-green">
              {shots.length} {shots.length === 1 ? 'view' : 'views'}
            </span>
          </div>
          <p className="text-sm text-lafoi-gray font-general mt-1.5 line-clamp-1 max-w-xl">{blurb}</p>
        </div>
        <Link
          to={`/products/${p.slug}`}
      ...
      {/* Prev / next controls — shown on laptop & desktop where there's no touch;
          hidden on mobile (touch swipe works). Disabled at the ends. */}
      {shots.length > 1 && (
        <div className="hidden sm:flex items-center gap-2 mt-2">
          <button
            ...
            onClick={() => scrollBy(-1)}
            disabled={atStart}
            aria-label={`Scroll ${p.finish} photos left`}
            className="w-10 h-10 rounded-full border border-lafoi-dark/15 bg-white text-lafoi-dark flex items-center justify-center hover:border-lafoi-green hover:text-lafoi-green transition-colors disabled:opacity-30 disabled:pointer-events-none"
          >
          ...
          <button
            ...
            onClick={() => scrollBy(1)}
            disabled={atEnd}
            aria-label={`Scroll ${p.finish} photos right`}
            className="w-10 h-10 rounded-full border border-lafoi-dark/15 bg-white text-lafoi-dark flex items-center justify-center hover:border-lafoi-green hover:text-lafoi-green transition-colors disabled:opacity-30 disabled:pointer-events-none"
          >
...
function ScrollStage({ stage, index, active }) {
  ...
  return (
    ...
    >
      ...
      <div className="lg:hidden mb-6 rounded-sm overflow-hidden aspect-[4/3]">
        <OptimizedImage
          src={stage.image}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 lg:gap-6">
          {stats.map((s, i) => (
            <AnimatedSection key={s.label} delay={i * 0.08}>
              <div className="conic-ring relative rounded-sm">
                <div className="neumorph-glass rounded-sm p-7 lg:p-8 h-full flex flex-col">
...
function BentoProject({ project, large = false }) {
  return (
    <Link
      ...
      className={`group relative block w-full rounded-sm overflow-hidden bg-lafoi-dark ${
    ...
    >
      ...
      <div className="absolute top-5 left-5 lg:top-6 lg:left-6">
        <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-sm bg-white/10 backdrop-blur-md border border-white/15 text-[10px] font-sora text-white/85 tracking-[0.2em] uppercase">
          {project.category}
        </span>
      </div>
      ...
      <div className="absolute inset-x-0 top-8 lg:top-10 z-20 pointer-events-none">
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 flex justify-end">
          <motion.div
            ...
            className="pointer-events-auto inline-flex items-center gap-2.5 px-4 py-2 rounded-sm bg-white/10 backdrop-blur-md border border-white/20"
          >
        ...
        <div className="max-w-4xl">
          ...
          <AnimatedSection delay={0.4}>
            <div className="mt-12 flex flex-wrap items-center gap-4 lg:gap-5">
              <Link
                to="/contact"
                className="group inline-flex items-center gap-3 px-8 py-4 rounded-sm bg-lafoi-green-light text-white font-sora text-sm font-semibold hover:bg-lafoi-green transition-all duration-500 shadow-[0_8px_30px_rgba(34,197,94,0.25)]"
              >
                Start your project
                <ArrowRight
                  size={16}
              ...
              <Link
                to="/projects"
                className="group inline-flex items-center gap-3 px-8 py-4 rounded-sm bg-white/8 backdrop-blur-md text-white font-sora text-sm font-semibold border border-white/25 hover:bg-white/12 hover:border-white/45 transition-all duration-500"
              >
                Explore portfolio
                <ArrowUpRight
                  size={16}
