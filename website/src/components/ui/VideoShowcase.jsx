...
function VideoCard({ video, onOpen, aspect = 'aspect-[16/10]', large = false, index = 0 }) {
  ...
  return (
    <motion.button
      ...
      className={`group relative block w-full ${aspect} rounded-sm overflow-hidden bg-lafoi-dark text-left ring-1 ring-lafoi-dark/10 hover:ring-lafoi-green-light/40 transition-shadow duration-500 shadow-[0_18px_40px_-28px_rgba(17,17,17,0.35)]`}
    ...
    >
      ...
      {/* index — top-left */}
      <span className="absolute top-4 left-4 inline-flex items-center gap-2 px-2.5 py-1 rounded-sm bg-white/10 backdrop-blur-md border border-white/15 text-[10px] font-sora text-white/85 tracking-[0.2em] uppercase">
        <span
          aria-hidden
...
function VideoModal({ video, onClose }) {
  ...
  return createPortal(
    ...
    >
      ...
      >
        <div className="relative rounded-sm overflow-hidden bg-lafoi-dark shadow-[0_40px_120px_-20px_rgba(0,0,0,0.6)] ring-1 ring-white/10">
          <video
            src={video.src}
            controls
            autoPlay
