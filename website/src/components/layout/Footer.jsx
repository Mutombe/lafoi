...
export default function Footer({ onOpenPolicy, onOpenPrivacy }) {
  ...
  return (
    <footer className="relative bg-lafoi-dark text-white overflow-hidden isolate">
            ...
            <AnimatedSection delay={0.1}>
              <form onSubmit={handleNewsletter} className="flex gap-3 max-w-md lg:ml-auto">
                <input
                  ...
                  className="flex-1 px-5 py-3.5 bg-white/[0.07] border border-white/[0.12] rounded-sm text-sm font-general text-white placeholder:text-white/30 outline-none focus:border-lafoi-green/50 focus:bg-white/10 transition-all duration-300"
                />
                <button
                  type="submit"
                  className="px-6 py-3.5 bg-lafoi-green text-white rounded-sm font-sora text-sm font-medium hover:bg-lafoi-green-light transition-all duration-300 flex items-center gap-2 group shrink-0 shadow-lg shadow-lafoi-green/20"
                >
                  Subscribe
      ...
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[720px] h-[260px] bg-lafoi-green/[0.06] rounded-sm blur-[140px] pointer-events-none" />
    </footer>
  )
}
